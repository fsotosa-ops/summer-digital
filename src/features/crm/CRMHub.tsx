  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row min-h-[60vh] md:h-[calc(100vh-12rem)] rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        {/* Mobile pill nav */}
        <nav className="md:hidden flex gap-2 overflow-x-auto p-2 border-b border-slate-100 bg-white">
          {navItems.map(({ value, label, icon: Icon }) => (
            <button
              key={value}
              onClick={() => setActiveSection(value)}
              className={cn(
                'flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-medium shrink-0 min-h-[44px] transition-colors',
                activeSection === value
