...
function LoginContent() {
  ...
  // 1. Extraer intención dinámica desde la URL
  const joinEventId = searchParams.get('join') || '';

  // Efecto 1: Preservar la intención de unirse al evento en la sesión
  ...
  // Nota: No hacemos router.push() aquí. Solo mutamos el estado de Zustand (user).
  // El "Efecto 3" detectará la mutación y ejecutará la redirección limpiamente.
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // El store (Zustand) expone el error, la UI lo renderiza automáticamente.
    }
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      window.location.href = oauthUrl; // Salida segura del entorno Next.js
    } catch {
